...
class ChordsPairDataset(Dataset):
    def __init__(
        self,
        split: str = "train",
        ...
        return_negative_audio: bool = True,
    ):
        self.root = Path(root)
        ...
        self.return_negative_audio = return_negative_audio
        ...
        # Global mapping, before split, so train/test use same indices
        all_programs = sorted(full_df["midi_program_num"].unique())
        self.program_to_class = {int(p): i for i, p in enumerate(all_programs)}
        self.class_to_program = {i: int(p) for p, i in self.program_to_class.items()}
        self.num_classes = len(all_programs)
        ...
        self.instrument_labels = torch.tensor(
            [self.program_to_class[int(p)] for p in self.df["midi_program_num"].values],
            dtype=torch.long,
        )

        self._preload_data_to_ram()

        # Build class -> indices mapping for negative sampling
        self.class_to_indices = {}
        for i, label in enumerate(self.instrument_labels.tolist()):
        ...
        if len(self.available_classes) < 2:
            raise ValueError("Need at least two instruments to sample a different-instrument audio.")
    ...
    def _cache_item(self, audio):
        ...
        audio = np.asarray(audio).squeeze()  # [1, N] -> [N]
    ...
    def sample_different_instrument_index(self, label):
        # Pick any class different from the current label
        other_classes = [c for c in self.available_classes if c != label]
        neg_class = np.random.choice(other_classes)

        # Pick a random item from that class
    ...
    def __getitem__(self, idx):
        label = int(self.instrument_labels[idx])

        audio = self._restore_cached_item(self.ram_cache[idx])
        audio = self.process_audio(audio)
        audio = torch.from_numpy(audio).float()

        if not self.return_negative_audio:
            return audio, self.instrument_labels[idx]

        neg_idx = self.sample_different_instrument_index(label)
        neg_label = int(self.instrument_labels[neg_idx])

        neg_audio = self._restore_cached_item(self.ram_cache[neg_idx])
        neg_audio = self.process_audio(neg_audio)
        neg_audio = torch.from_numpy(neg_audio).float()

        return audio, neg_audio, self.instrument_labels[idx], self.instrument_labels[neg_idx]